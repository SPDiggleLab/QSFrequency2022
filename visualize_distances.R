...
if(!dir.exists('OBJECTS/temp')){
  # Needed for saving PCAs
  dir.create('OBJECTS/temp')
}
...
for(gene in names(ls.distMats_by_gene)){
  ...
  , error = function(cond) {
    ...
    save(temp.pcas, file = paste('./OBJECTS/temp/PCA_', gene, '.R', sep=''))
    ggsave(filename = paste('./OUTPUT/PCAs/pca_', gene, '.png', sep='', collapse = '')
           , plot = p_pca
           , height = 5
           , width = 5
...
for(gene_pca in names(ls.distMats_by_gene)[!names(ls.distMats_by_gene) %in% names(ls.pcas)]){
  load(paste('./OBJECTS/temp/PCA_', gene_pca, '.R', sep=''))
